...
// Função para garantir que o diretório "Data" e o arquivo "seq_minmaxsort.txt" existam
void garantirDiretorioEArquivo() {
    ...
    // Criar ou abrir o arquivo Data/seq_minmaxsort.txt para adicionar o log do tempo
    FILE *arquivoLog = fopen("Data/seq_minmaxsort.txt", "a");
    if (!arquivoLog) {
...
void registrarTempoNoArquivo(double tempoGasto, int comprimentoA) {
    ...
    // Adicionar a linha de log no arquivo Data/seq_minmaxsort.txt
