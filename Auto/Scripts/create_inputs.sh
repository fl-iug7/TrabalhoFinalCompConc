...
if [[ ! -f "$programa_compilado" ]]; then
    ...
    read -p "O programa $programa já está compilado. Você deseja compilá-lo novamente? (s/n): " resposta
    if [[ "$resposta" == "s" || "$resposta" == "S" ]]; then
        compilar_programa
    elif [[ "$resposta" == "n" || "$resposta" == "N" ]]; then
        echo "Prosseguindo com a execução do programa já compilado..."
        echo "--------------------------------------------------"
    else
        echo "Resposta inválida. A execução foi cancelada."
        echo "--------------------------------------------------"
        exit 1
    fi
fi
